...
{
    [AddComponentMenu(UIAniConst.ComponentMenuBasePath + "RectTransform/Anchored Position")]
    public class RectTransformAnchoredPositionTween : PingPongTweenRxComponentBase<RectTransform, Vector2>
    {
        ...
        public override void Ready()
        {
            ...
            if (Target == null)
            {
                Debug.LogError($"[TinaX.Tween]{nameof(RectTransformAnchoredPositionTween)} cannot get valid target.");
                valid_tween = false;
            }

            if (!this._AutoOriginValue)
            {
                this.Target.anchoredPosition = this._FromValue;
            }
            else
            {
            ...
            origin_value = this._AutoOriginValue ? this.Target.anchoredPosition : this._FromValue;
            target_value = this._AutoTargetValue ? this.Target.anchoredPosition : this._ToValue;
        ...
        public override void BeginPlay()
        {
            ...
            this.TweenRxDisposable = TinaX.Tween.Tween.Play(
                    ...
                    this._EaseType,
                    this.DelayBefore)
                    .Subscribe(value => { this.Target.anchoredPosition = value; }, tweenFinish)
                    .AddTo(this.Target);
        }
        ...
        private void tweenFinish()
        {
            if (this.PingPong)
            {
                ...
                this.TweenRxDisposable = obsv3.Subscribe(value => { this.Target.anchoredPosition = value; }, tweenFinish)
                        .AddTo(this.Target);
            }
            else
            {
