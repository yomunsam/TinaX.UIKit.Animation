...
{
    [AddComponentMenu(UIAniConst.ComponentMenuBasePath + "RectTransform/Size Delta Tween")]
    public class RectTransformSizeDeltaTween : PingPongTweenRxComponentBase<RectTransform, Vector2>
    {
        ...
        public override void Ready()
        {
            ...
            if (Target == null)
            {
                Debug.LogError($"[TinaX.Tween]{nameof(RectTransformSizeDeltaTween)} cannot get valid target.");
                valid_tween = false;
            }

            if (!this._AutoOriginValue)
            {
                this.Target.sizeDelta = this._FromValue;
            }
            else
            {
            ...
            origin_value = this._AutoOriginValue ? this.Target.sizeDelta : this._FromValue;
            target_value = this._AutoTargetValue ? this.Target.sizeDelta : this._ToValue;
        ...
        public override void BeginPlay()
        {
            ...
            this.TweenRxDisposable = TinaX.Tween.Tween.Play(
                    ...
                    this._EaseType,
                    this.DelayBefore)
                    .Subscribe(value => { this.Target.sizeDelta = value; }, tweenFinish)
                    .AddTo(this.Target);
        }
        ...
        private void tweenFinish()
        {
            if (this.PingPong)
            {
                ...
                this.TweenRxDisposable = obsv3.Subscribe(value => { this.Target.sizeDelta = value; }, tweenFinish)
                        .AddTo(this.Target);
            }
            else
            {
