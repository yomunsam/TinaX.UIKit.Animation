...
{
    [AddComponentMenu(UIAniConst.ComponentMenuBasePath + "Canvas Group/Canvas Group Alpha Tween")]
    public class CanvasGroupAlphaTween : PingPongTweenRxComponentBase<CanvasGroup, float>
    {
        private float? origin_value;
        ...
        public override CanvasGroup GetDefaultTarget()
        {
            if (this == null)
                return null;
        ...
        public override void Ready()
        {
            ...
            if (Target == null)
            {
                Debug.LogError($"[TinaX.Tween]{nameof(CanvasGroupAlphaTween)} cannot get valid target.");
                valid_tween = false;
            }

            if (!this._AutoOriginValue)
            {
                this.Target.alpha = this._FromValue;
            }
            else
            {
            ...
            origin_value = this._AutoOriginValue ? this.Target.alpha : this._FromValue;
            target_value = this._AutoTargetValue ? this.Target.alpha : this._ToValue;
        ...
        public override void BeginPlay()
        {
            ...
            this.TweenRxDisposable = TinaX.Tween.Tween.Play(
                    ...
                    this._EaseType,
                    this.DelayBefore)
                    .Subscribe(value => { this.Target.alpha = value; }, tweenFinish)
                    .AddTo(this.Target);
        }
        ...
        private void tweenFinish()
        {
            if (this.PingPong)
            {
                ...
                this.TweenRxDisposable = obsv3.Subscribe(value => { this.Target.alpha = value; }, tweenFinish)
                        .AddTo(this.Target);
            }
            else
            {
